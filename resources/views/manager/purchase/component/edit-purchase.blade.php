                ...
                <form action="{{ route('purchase.update', $purchase->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-6">
                        ...
                        @if($purchase->attachment)
                            <div class="mb-2">
                                <a href="{{ asset('storage/' . $purchase->attachment) }}" target="_blank" class="text-blue-500 hover:underline">
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200" id="itemsTable">
                                ...
                                <tbody id="itemRows" class="bg-white divide-y divide-gray-200">
                                    @php
                                        $oldItems = old('items', $purchase->items->isEmpty() ? [['product_id' => '', 'quantity' => 1, 'price' => 0]] : $purchase->items->toArray());
                                    @endphp
                                    ...
                                    <tr class="item-row">
                                            ...
                                            <select name="items[{{ $i }}][product_id]" required
                                                ...
                                                @foreach($products as $product)
                                                    <option value="{{ $product->id }}" 
                                                        {{ (isset($item['product_id']) ? $item['product_id'] : $item['product']['id']) == $product->id ? 'selected' : '' }}>
                                                        {{ $product->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        <td class="px-4 py-2 whitespace-nowrap">
                                            <span class="total">{{ ($item['quantity'] ?? 1) * ($item['price'] ?? 0) }}</span>
                                        </td>
                                        <td class="px-4 py-2 whitespace-nowrap">
                                            <button type="button"
                                ...
                                <tfoot>
                                    <tr>
                                        ...
                                        <td id="subtotal" class="px-4 py-2 font-semibold">{{ $purchase->amount }}</td>
                                        <td></td>
                                    </tr>
                                </tfoot>
                            </table>
